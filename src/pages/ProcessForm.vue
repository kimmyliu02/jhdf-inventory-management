...
import { getAll, addRecord, writeLedger, getLiveStock, getAllStock, genNo } from '../db/index.js'
...
// Available batches + stock for selected raw material
const availableBatches = ref([])   // [{ batchNo, qty }]
const inBatchNo        = ref('')
const liveStock        = ref(null) // stock of selected batch
...
// Unit is from the product record, not hardcoded
...
// When raw material changes, load available batches
watch(inProductId, async (id) => {
  ...
  availableBatches.value = allStock
    .filter(s => s.productId === Number(id) && s.qty > 0)
    .sort((a, b) => a.batchNo.localeCompare(b.batchNo))
  // Auto-select first batch if only one
  if (availableBatches.value.length === 1) {
    inBatchNo.value = availableBatches.value[0].batchNo
  }
})

// When batch changes, update live stock display
...
const stockHintClass = computed(() => {
  if (liveStock.value === null) return ''
  const q = parseInt(inQty.value) || 0
  if (q > liveStock.value) return 'hint-bad'
  if (q > 0) return 'hint-ok'
  return ''
})
...
async function submit() {
  ...
  const stock = await getLiveStock(Number(inProductId.value), inBatchNo.value)
  if (qi > stock) return alert(`原料库存不足（当前 ${stock} ${inUnit.value}）`)
  ...
  await addRecord('processing', {
    procNo,
    inProductId:  inProduct.value.id,  inProductName:  inProduct.value.name,
    inBatchNo:    inBatchNo.value,      inQty:          qi,
    outProductId: outProduct.value.id, outProductName: outProduct.value.name,
    outBatchNo,                         outQty:         qo,
  })
  await writeLedger({
    productId: inProduct.value.id, productName: inProduct.value.name,
    batchNo: inBatchNo.value, type: 'process_consume', qtyChange: -qi, refNo: procNo,
  })
  await writeLedger({
    productId: outProduct.value.id, productName: outProduct.value.name,
    batchNo: outBatchNo, type: 'process_produce', qtyChange: qo, refNo: procNo,
  })
  ...
  <div class="screen">
    ...
    <template v-if="done">
      ...
      <div class="success-wrap">
        <div class="success-icon" style="background:var(--amber-light);color:var(--amber)">
          <i class="ti ti-refresh" />
        </div>
        <div class="success-title">加工记录已提交</div>
        <div class="success-sub">库存已同步更新</div>
        <div class="success-tag" style="background:var(--amber-light);color:var(--amber-dark)">
          加工单号：{{ result.procNo }}
        </div>
    ...
    <template v-else>
      ...
      <div class="body">
        ...
        <!-- Batch selector with stock shown -->
        <div class="field-group">
        ...
        <div v-if="liveStock !== null" style="margin-bottom:14px">
          <div :class="['hint-box', parseInt(inQty) > liveStock ? 'hint-bad' : 'hint-ok']">
            ...
            <span v-if="parseInt(inQty) > 0 && parseInt(inQty) <= liveStock" style="margin-left:6px;color:var(--teal-dark)">
              · 消耗后剩余 {{ liveStock - parseInt(inQty) }} {{ inUnit }}
            </span>
          </div>
        </div>

        <!-- Consume qty — unit from product -->
        <div class="field-group">
          ...
          <div style="display:flex;gap:8px;align-items:center">
            <input
              ...
              v-model="inQty"
              style="flex:1"
              :style="{ borderColor: parseInt(inQty) > liveStock ? 'var(--red)' : '' }"
            >
            <span style="font-size:13px;color:var(--text2);white-space:nowrap">{{ inUnit }}</span>
          </div>
        </div>
        ...
        <button class="btn btn-amber" @click="submit">
          <i class="ti ti-check" />提交加工记录
        </button>
        <button class="btn btn-ghost" @click="router.push('/')">返回首页</button>
      </div>
    </template>
