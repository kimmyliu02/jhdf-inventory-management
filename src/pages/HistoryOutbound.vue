...
import { getOutboundHistory } from '../api/index.js'
...
onMounted(async () => {
  try {
    records.value = await getOutboundHistory()
  } catch (e) {
...
<template>
  <div class="screen">
    <div class="topbar">
      ...
      <span class="topbar-title">出库历史</span>
    </div>
    ...
    <div class="body">
      ...
      <div v-else-if="records.length === 0" class="empty"><i class="ti ti-inbox" />暂无出库记录</div>
      ...
      >
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
          ...
          <span class="badge badge-purple">−{{ r.qty_actual }} 出库</span>
        </div>
        <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:4px 12px">
          <div class="info-row"><span class="info-key">出库单号</span><span class="info-val">{{ r.outbound_no }}</span></div>
          <div class="info-row"><span class="info-key">销售单号</span><span class="info-val">{{ r.sales_order_no || '—' }}</span></div>
          <div class="info-row"><span class="info-key">购货方</span><span class="info-val">{{ r.buyer || '—' }}</span></div>
          <div class="info-row"><span class="info-key">应发 / 实发</span><span class="info-val">{{ r.qty_ordered }} / {{ r.qty_actual }}</span></div>
          <div class="info-row"><span class="info-key">承运方式</span><span class="info-val">{{ r.transport || '—' }}</span></div>
