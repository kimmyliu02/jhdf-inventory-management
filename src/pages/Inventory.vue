...
import { getAll, getAllStock } from '../db/index.js'
...
async function load() {
  const [prods, stock] = await Promise.all([getAll('products'), getAllStock()])
  prodMap.value = Object.fromEntries(prods.map(p => [p.id, p]))
  allRows.value = stock
    .map(s => ({ ...s, product: prodMap.value[s.productId] }))
    .filter(s => s.product)
}
...
const visible = computed(() => {
  return allRows.value.filter(r => {
    ...
    if (search.value) {
      ...
      if (!r.product.name.includes(search.value) && !r.batchNo.toLowerCase().includes(q)) return false
    }
    return true
  })
})
  ...
  <div class="screen">
    ...
    <div class="body">
      ...
      <div v-if="visible.length === 0" class="empty">
        <i class="ti ti-package-off" />没有符合条件的商品
      </div>

      <!-- Inventory cards -->
      <div
        v-for="r in visible"
        :key="`${r.productId}-${r.batchNo}`"
        :style="{
          background:'var(--bg2)',
